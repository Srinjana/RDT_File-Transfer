...
public class receiver {
    ...
    public receiver(InetAddress address, Integer port1, Integer port2, File file) throws IOException {
    ...
    private void start() throws Exception {
        while (endOfFile != 1) {
            // cleans the byte buffer in case the received data is smaller than the previous
            // one.
                ...
                case 1:
                    ...
                    if (p.getData().length < 0) {
                        ...
                        if (p.getSeqno() == nextSeqNum) {
                            ...
                            System.out.println("Attempting to send ACK packet for correct sequence # packet: "
                                    + p.getSeqno() % modulo);
                            ...
                            System.out.println("Warning! Received packet out of sequence! Expected packet number: "
                                    + nextSeqNum + " but got: " + p.getSeqno());

                            if (nextSeqNum == 0)
                                break;
                            else {
                                // Send ACK to sender, but its not the packet we need
                                System.out.println("Attempting to send ACK packet for Out of Order packet: "
                                        + p.getSeqno() % modulo);
                                sendACK(p.getSeqno() % modulo);
                            }
    ...
    private void sendEOT(int seqNum) throws Exception {
        ...
        DatagramPacket eotPacket = new DatagramPacket(eotBuffer, eotBuffer.length, targetHostName, theSenderPort);
    ...
    private void sendACK(int seqNum) throws Exception {
        ...
        if (randomNumber <= errorRate) {
            ...
            DatagramPacket ackPacket = new DatagramPacket(ackBuffer, ackBuffer.length, targetHostName, theSenderPort);
    ...
    public static void main(String args[]) throws IOException, Exception {
        ...
        receiver newReceiver = new receiver(hostAddress, senderPort, receiverPort, writtenFile);
