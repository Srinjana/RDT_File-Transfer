...
/**
 * This class plays the role of the client aka sender and sends a text file to
 * the receiver. It listens on a predefined port and get ACK packets from the
 * receiver proving they were received. If no ACK packet is received it will
 * keep resending the packets.
 */
public class sender {
    ...
    private static void createThreads(final InetAddress addr, final int port, final FileInputStream fis,
            final DatagramSocket socket) throws InterruptedException {
    ...
    private static void packetSend(FileInputStream fis, InetAddress target, int port, DatagramSocket socket)
            throws Exception {
        ...
        try {
            ...
            while (true) {
                ...
                if (nextSeqNum >= startingNumber + windowSize) {
                    // System.out.println("Sliding window full, waiting for ACK: " +
                    // startingNumber);
                    // System.out.println("NexSeqNum is: " + nextSeqNum + " and starting number +
                    // window size " + windowSize + " is: " + (startingNumber + windowSize));
                ...
                synchronized (lock) {
                    ...
                    if (result < 0) {
                        ...
                        DatagramPacket senderPacket = new DatagramPacket(p.getUDPdata(), p.getUDPdata().length, target,
                                port);
                        ...
                        System.out.println("");
                        System.out.println(
                                "[" + timeStampEOT + "] Sending last packet EOT: " + p.getSeqno() + " to receiver");
                        ...
                        // Only really changes anything on the last packet. Any other packet result
                        // should be 500.
                        ...
                        if (errorRate > 0) {
                            ...
                            // System.out.println("RandomNumber for packet " + nextSeqNum + " is: " +
                            // randomNumber);
                        }
                        ...
                        if (randomNumber <= errorRate) {
                            ...
                            DatagramPacket senderPacket = new DatagramPacket(p.getUDPdata(), result, target, port);
                            String timeStampLosePacket = new SimpleDateFormat("HH.mm.ss").format(new Date());
                            System.out.println("");
                            System.out.println(" *[" + timeStampLosePacket + "] Losing packet: " + p.getSeqno()
                                    + " sent to receiver!... ");
                            ...
                            p = new packet(nextSeqNum, nextSeqNum, buffer);
                            DatagramPacket senderPacket = new DatagramPacket(p.getUDPdata(), p.getUDPdata().length,
                                    target, port);
                            socket.send(senderPacket);
    ...
    private static void acksMonitor(InetAddress target, int port, DatagramSocket socket) throws Exception {

        while (true) {

            try {
                // An Ack packet only holds 8 bytes so the buffer holds 8 to receive that
                // amount.
                byte[] buffer = new byte[8];
                // set socket timeout
                socket.setSoTimeout(timeout);
                // set the socket to receive ACKs
                DatagramPacket ACKs = new DatagramPacket(buffer, buffer.length, target, port);
                socket.receive(ACKs);
                ...
                if (ACKPackets.getType() == 2) {
                    ...
                    // System.out.println(" -Removing EOT " + ACKPackets.getSeqNum() + " from
                    // unAckedArray");
                ...
                System.out.println("");
                System.out.println(" ![" + timeStampReceiveAck + "] Received ACK for packet: " + ACKPackets.getAckno()
                        + " from receiver");
                ...
                synchronized (lock) {
                    ...
                    if (ACKPackets.getAckno() == 0) {
                        ...
                        // System.out.println(" -Removing packet " + ACKPackets.getAckno() + " from
                        // unAckedArray");
                        ...
                        System.out.println("Warning! previous packet was not ACKed, need ACK from receiver for packet: "
                                + (ACKPackets.getAckno() - 1));
                        // outputAckArray();
                        ...
                        // System.out.println(" -Removing " + ACKPackets.getAckno() + " from
                        // unAckedArray");
                        unAckedArray[ACKPackets.getAckno()] = null;
                        // outputAckArray();
                ...
                synchronized (lock) {
                    ...
                    for (int i = 0; i < modulo; i++) {
                        ...
                        if (errorRate > 0) {
                            ...
                            Random randomGenerator = new Random();
                            randomNumber = randomGenerator.nextInt(100);
                            // System.out.println("RandomNumber for packet resend: " +
                            // cache.get(i).getSeqNum() + " is: " + randomNumber);
                        }
                        ...
                        if (randomNumber <= errorRate) {
                            // Drop the resend
                            @SuppressWarnings("unused")
                            DatagramPacket dp = new DatagramPacket(unAckedArray[i].getUDPdata(),
                                    unAckedArray[i].getUDPdata().length, target, port);
                            String timeStampBadResend = new SimpleDateFormat("HH.mm.ss").format(new Date());
                            System.out.println("");
                            System.out.println(" *[" + timeStampBadResend + "] Packet Timeout: Dropping resend of: "
                                    + unAckedArray[i].getSeqno());

                            // Dont actually send the resend
                            // socket.send(dp);

                            // write to log file
                            sequenceNumberFile.write((Integer.toString(unAckedArray[i].getSeqno()) + '\n').getBytes());
                            ...
                            // unAckedList.get(i).getSeqNum());
                            System.out.println("");
                            String timeStampGoodResend = new SimpleDateFormat("HH.mm.ss").format(new Date());
                            System.out.println("[" + timeStampGoodResend + "]Packet Timeout: Correctly resending: "
                                    + unAckedArray[i].getSeqno());
                        }
                    }
                }
    ...
    public static void setTimeout() {
        timeout = Integer.parseInt(stringReader(
                "Please enter the time in milliseconds that the packets should timeout before being acknowleged (e.g. 3000)  \n"));
    }
